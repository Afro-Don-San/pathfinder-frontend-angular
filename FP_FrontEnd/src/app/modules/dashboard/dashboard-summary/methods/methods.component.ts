...
export class MethodsComponent implements OnInit {
  ...
  updateCard1Chart(filter: { from_date, to_date, facilities }) {
    ...
    const reportUrl = 'service_provided/';
    this.http.postDJANGOURL(reportUrl, filter)
        .subscribe((data: any[]) => {
          if (data) {
            ...
            const categories = data.map(item => item.service_provided);
            const chartConfig: any = this.settingsService.drawChart(
                categories,
                series,
                'Client Count',
                'Services provided ' + ` from ${filter.from_date} to ${filter.to_date} for ${this.orgunitName}`,
                // '',
                // 'line'
            );
