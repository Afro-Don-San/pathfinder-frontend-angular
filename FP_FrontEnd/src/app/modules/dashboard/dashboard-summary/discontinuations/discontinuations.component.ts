...
@Component({
  ...
  templateUrl: './discontinuations.component.html',
  styleUrls: ['./discontinuations.component.scss']
})
export class DiscontinuationsComponent implements OnInit {
    ...
    this.http.postDJANGOURL(reportUrl, filter)
        .subscribe((data: any[]) => {
          if (data) {
            const series = [{
              name: 'Months',
              data: data['family_planning_discontinuations'].map(item => item.value)
            }];
            const categories = data['family_planning_discontinuations'].map(item => item.month_name);
            const chartConfig: any = this.settingsService.drawChart(
                categories,
                series,
                'Client Count',
                'Family Planning Discontinuations' + ` from ${filter.from_date} to ${filter.to_date} for ${this.orgunitName}`,
                // '',
                // 'line'
            );
