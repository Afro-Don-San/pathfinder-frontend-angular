    ...
    this.http.postDJANGOURL(reportUrl, filter)
        .subscribe((data: any[]) => {
          if (data) {
            ...
            const series = [{
              name: 'Visit Types',
              data: data['total_fp_visits'].map(item => item.value)
            }];
            const categories = data['total_fp_visits'].map(item => item.event_type);
            const chartConfig: any = this.settingsService.drawChart(
                categories,
                series,
                'Client Count',
                'Total Visits' + ` from ${filter.from_date} to ${filter.to_date} for ${this.orgunitName}`,
                // '',
                // 'line'
            );
